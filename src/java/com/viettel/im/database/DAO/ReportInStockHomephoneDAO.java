...
import com.viettel.im.client.form.ReportInStockHomephoneForm;
 ...
 *
 * @author TheTM
 * xu ly cac tac vu lien quan den bao cao so ton kho Homephone
 *
 */
public class ReportInStockHomephoneDAO extends BaseDAOAction {

    private Log log = LogFactory.getLog(ReportNiceNumberDAO.class);
    //dinh nghia cac hang so pageForward
    private String pageForward;
    private final String REPORT_IN_STOCK_HOMEPHONE = "reportInStockHomephone";
    ...
    //khai bao bien form
    ReportInStockHomephoneForm reportInStockHomephoneForm = new ReportInStockHomephoneForm();

    public ReportInStockHomephoneForm getReportInStockHomephoneForm() {
        return reportInStockHomephoneForm;
    }

    public void setReportInStockHomephoneForm(ReportInStockHomephoneForm reportInStockHomephoneForm) {
        this.reportInStockHomephoneForm = reportInStockHomephoneForm;
    }
    ...
    public String preparePage() throws Exception {
        ...
        this.reportInStockHomephoneForm.resetForm();

        //thiet lap gia tri mac dinh ban dau la stockIsdnMobile
        this.reportInStockHomephoneForm.setStockTypeId(Constant.STOCK_ISDN_MOBILE);

        //lay du lieu cho cac combobox
        getDataForCombobox();

        pageForward = REPORT_IN_STOCK_HOMEPHONE;
    ...
    private void getDataForCombobox() {
        ...
        listProvince = area.findAllProvince();
        req.setAttribute(REQUEST_LIST_PROVINCE, listProvince);

        Long stockTypeId = this.reportInStockHomephoneForm.getStockTypeId();
        if (stockTypeId != null) {
            ...
            String groupFilterRuleCode = this.reportInStockHomephoneForm.getGroupFilterRuleCode();
            if (groupFilterRuleCode != null && !groupFilterRuleCode.trim().equals("")) {
                ...
                Long filterTypeId = this.reportInStockHomephoneForm.getFilterTypeId();
    ...
    public String reportNiceNumber() throws Exception {
        ...
        Long stockTypeId = this.reportInStockHomephoneForm.getStockTypeId();
        String isdnType = this.reportInStockHomephoneForm.getIsdnType();
        Long status = this.reportInStockHomephoneForm.getStatus();
        String groupFilterRuleCode = this.reportInStockHomephoneForm.getGroupFilterRuleCode();
        Long filterTypeId = this.reportInStockHomephoneForm.getFilterTypeId();
        Long ruleId = this.reportInStockHomephoneForm.getRuleId();
        Long fromPrice = this.reportInStockHomephoneForm.getFromPrice();
        Long toPrice = this.reportInStockHomephoneForm.getToPrice();
        Long stockModelId = this.reportInStockHomephoneForm.getStockModelId();
        String shopCode = this.reportInStockHomephoneForm.getShopCode();
        Long shopId = this.reportInStockHomephoneForm.getShopId();
        String strFromIsdn = this.reportInStockHomephoneForm.getFromIsdn();
        String strToIsdn = this.reportInStockHomephoneForm.getToIsdn();
...
//Lay ma kho so
        ...
        try {
            ...
            request.set("SHOP_NAME", reportInStockHomephoneForm.getShopName());
...
//             nhớ sửa   request.set(ReportConstant.REPORT_KIND, ReportConstant.IM_REPORT_IN_STOCK_HOMEPHONE);
            ...
            if (response != null && response.get(ReportConstant.RESULT_FILE) != null && !"".equals(response.get(ReportConstant.RESULT_FILE).toString())) {
                //Thong bao len jsp
                reportInStockHomephoneForm.setPathExpFile(response.get(ReportConstant.RESULT_FILE).toString());
...
//Cac dieu kien tim kiem
        ...
        if (strFromIsdn != null && !strFromIsdn.trim().equals("")) {
            try {
                ...
                this.reportInStockHomephoneForm.setFromIsdn(strFromIsdn.trim());
            } catch (NumberFormatException ex) {
                ex.printStackTrace();
        ...
        if (strToIsdn != null && !strToIsdn.trim().equals("")) {
            try {
                ...
                this.reportInStockHomephoneForm.setToIsdn(strToIsdn.trim());
            } catch (NumberFormatException ex) {
                ex.printStackTrace();
                //bao loi so isdn khong dung dinh dang
...
//            throw new Exception("Danh sách số đẹp rỗng");
            ...
            reportInStockHomephoneForm.setPathExpFile(filePath);
            ...
            if (reportInStockHomephoneForm.getShopName() != null || !reportInStockHomephoneForm.getShopName().equals("")) {
                beans.put("shopName", reportInStockHomephoneForm.getShopName());
