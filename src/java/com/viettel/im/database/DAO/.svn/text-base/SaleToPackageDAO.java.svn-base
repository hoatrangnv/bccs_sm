 ...
 * @author MrSun
 */
public class SaleToPackageDAO extends BaseDAOAction {

    private SaleTransForm saleTransForm;
    private static final Long MAX_SEARCH_RESULT = 100L;
    private static final String SALE_TO_RETAIL = "saleToPackage";
    private static final String SALE_TO_RETAIL_DETAIL = "saleToPackageDetail";
...
//        }
    ...
    private void getDataForCombobox() {
        ...
        //QuocDM1 20150603 bo sung phan hien thi ly do thay cho phan hien thi nhu cu cua ToanCX
    ...
    public String updateListModelPrice() throws Exception {
        ...
        try {
            ...
            boolean allowSalePackage = checkProductSaleToPackage(stockModelF9.getStockModelId(), true);
            if (!allowSalePackage) {
                List listItems = new ArrayList();
                String[] header = {"", getText("MSG.SAE.024")};
                listItems.add(header);
            ...
            //QuocDM1 20150522 bo sung them tham so de lay gia ban theo goi R8388.
            boolean getPricePackage = true;
            AppParamsDAO appParamsDAO = new AppParamsDAO();
            appParamsDAO.setSession(getSession());
            /*
             Long TypeBranch = 0L;
             ...
             */
            List<AppParams> lstParams = appParamsDAO.findAppParamsByType("VIP_PRICE_PACKAGE_AVOID");
            //Neu co khai bao cau hinh chi lay gia ban bo thi gan = true
            for (int i = 0; i < lstParams.size(); i++) {
                if (stockModelF9.getStockTypeId().toString().equals(lstParams.get(i).getValue())) {
                    getPricePackage = false;
                }
            }

            if (reasonId != null && reasonId.equals(getReasonIdVipCustomer()) && getPricePackage) {
                //Lay gia ban le khi ban cho khach hang VIP
                priceList = priceDAO.findManyPriceForSaleRetail();
            } else {
                /*
                 //Them phan lay gia theo chinh sach gia cho showroom
                 ...
                 && priceDAO.findPriceForBranch(TypeBranch).size() > 0 ) {
                 priceList = priceDAO.findPriceForBranch(TypeBranch);
                 }
                 else if (staff != null && checkUserOfSH(getSession(), staff.getStaffId()) && priceDAO.findPriceOnlyShowroom().size() > 0) {
                 priceList = priceDAO.findPriceOnlyShowroom();
                 } else {
                 priceList = priceDAO.findManyPriceForSalePackage();
                 }
                 */
                //Lay gia ban theo goi khi ban cho khach hang thuong
                priceList = priceDAO.findManyPriceForSalePackage();
            }
    ...
    public List<ImSearchBean> getListStockModel(ImSearchBean imSearchBean) {
        ...
        strQuery1.append("and stock_model_id in ( select  pgd.stock_model_id ");
    ...
    private List<Price> getListPrice(Long stockModelId) throws Exception {
        ...
        StockModelDAO stockModelDAOF9 = new StockModelDAO();
        stockModelDAOF9.setSession(this.getSession());
        StockModel stockModelF9 = stockModelDAOF9.findById(stockModelId);
        List<AppParams> lstParams = appParamsDAO.findAppParamsByType("VIP_PRICE_PACKAGE_AVOID");
        for (int i = 0; i < lstParams.size(); i++) {
            if (stockModelF9.getStockTypeId().toString().equals(lstParams.get(i).getValue())) {
                getPricePackage = false;
         ...
         */
        if (isSaleToVipCustomer() && getPricePackage) {
            ...
            /*
             ...
             || listShopBranchMap.containsKey(staff.getShopId().toString()))
             && priceDAO.findPriceForBranch(TypeBranch).size() > 0 ) {
             listPrice = priceDAO.findPriceForBranch(TypeBranch);
             }
             else if (staff != null && checkUserOfSH(getSession(), staff.getStaffId()) && priceDAO.findPriceOnlyShowroom().size() > 0) {
             listPrice = priceDAO.findPriceOnlyShowroom();
             } else {
             listPrice = priceDAO.findManyPriceForSalePackage();
             }
             */
            listPrice = priceDAO.findManyPriceForSalePackage();
        }
...
//        }
    ...
    public String saveSaleTransToRetail() throws Exception {
        ...
        UserToken userToken = (UserToken) req.getSession().getAttribute(Constant.USER_TOKEN);
        getDataForCombobox();
        logdetail.info("Begin Method saveSaleTransToPackage :..........");
...
//                if (isStaffMaputo(userToken.getLoginName())) {
                    ...
                    if ((sumSaleTrans + sumDeposit + sumPayment + saleTransForm.getAmountAfterTax()) > limitMoney) {
...
//                }
                ...
                logdetail.info("SaleTransToPackage: " + "CustName: " + this.saleTransForm.getCustName() + "isdnWallet " + this.saleTransForm.getIsdnWallet() + "saveSaleTrans");
                ...
                if (this.saleTransForm.getPayMethod() != null
                    ...
                    if (saleTrans.getSaleTransId() > 0) {
                        logdetail.info("SaleTransToPackage: " + "CustName: " + this.saleTransForm.getCustName() + "isdnWallet " + this.saleTransForm.getIsdnWallet() + "saleTransEmola");
                        ...
                        if (saleTransEmola == null || saleTransEmola.getSaleTransId() == null) {
                            logdetail.info("SaleTransToPackage: saleTransEmola is fail. saleTransId :" + saleTrans.getSaleTransId());
                            req.setAttribute(Constant.RETURN_MESSAGE, "Error. Can not save sale transaction!");
                            this.getSession().clear();
                            this.getSession().getTransaction().rollback();
                            this.getSession().getTransaction().begin();
                        } else {
                            logdetail.info("SaleTransToPackage: saleTransEmola is success. saleTransId: " + saleTrans.getSaleTransId());
                ...
                /*luu giao dich vip*/
                if (isSaleToVipCustomer()) {
                    logdetail.info("SaleTransToPackage: saveSaleTransVip ");
                    SaleTransVip saleTransVip = this.saveSaleTransVip(saleTrans);
                    if (saleTransVip == null || saleTransVip.getSaleTransVipId() == null) {
                        logdetail.info("SaleTransToPackage: saveSaleTransVip Error. saleTransId: " + saleTrans.getSaleTransId());
                ...
                for (SaleTransDetailV2Bean bean : lstGoods) {
                    logdetail.info("SaleTransToPackage: saveSaleTransDetail ");
                    ...
                    if (saleTransDetail == null || saleTransDetail.getSaleTransId() == null) {
                        logdetail.info("SaleTransToPackage: saveSaleTransDetail fail. saletransId" + saleTrans.getSaleTransId());
                    ...
                    if (bean.getCheckSerial() != null && bean.getCheckSerial().equals(1L)) {
                        logdetail.info("SaleTransToPackage: Serial list empty. saletransId" + saleTrans.getSaleTransId());
                        ...
                        if (!saveSaleTransSerial(saleTransDetail, bean.getLstSerial())) {
                            logdetail.info("SaleTransToPackage: saveSaleTransSerial fail. saletransId" + saleTrans.getSaleTransId());
                    ...
                    if (bean.getStockModelCode() != null && bean.getStockModelCode().equals(Constant.STOCK_MODEL_CODE_TCDT)) {
                        logdetail.info("SaleTransToPackage: dont accept Anypay " + saleTrans.getSaleTransId());
                        getSession().clear();
                ...
                if (this.saleTransForm.getPayMethod() != null
                    ...
                    if (saleTrans.getSaleTransId() > 0) {
                        ...
                        logdetail.info("SaleTransToPackage: Start Call API Payment Keeto.saletransId" + saleTrans.getSaleTransId());
                        if (saleTransEmolaUpdate != null) {
                            ...
                            if ("ERROR".equals(response)) {
                                ...
                                logdetail.info("SaleTransToPackage: Error Call API Payment Keeto .saletransId" + saleTrans.getSaleTransId());
                                this.getSession().clear();
                                this.getSession().getTransaction().rollback();
                                this.getSession().getTransaction().begin();
                                //Update trang thai Sale Trans va Sale Trans Emola
                                logdetail.info("SaleTransToPackage: Start Update status Error:" + saleTrans.getSaleTransId());
                                ...
                                logdetail.info("SaleTransToPackage: End Update status Error:" + saleTrans.getSaleTransId());
                                return pageForward;
                            } else {
                                Gson gson = new Gson();
                                ...
                                if (responseWallet != null && responseWallet.getResponseCode() != null) {
                                    if (responseWallet.getResponseCode().equals("01")) {
                                         ...
                                         */
                                        logdetail.info("SaleTransToPackage: Start Update status success:" + saleTrans.getSaleTransId());
                                        ...
                                        this.getSession().update(saleTrans);
                                        logdetail.info("SaleTransToPackage: End Update status success:" + saleTrans.getSaleTransId());
                                    } else {
                                         ...
                                         */
                                        //rollback nghiep vu cua ban hang
                                        logdetail.info("SaleTransToPackage: Error Payment .saletransId" + saleTrans.getSaleTransId());
                                        this.getSession().clear();
                                        this.getSession().getTransaction().rollback();
                                        this.getSession().getTransaction().begin();
                                        //Update trang thai Sale Trans va Sale Trans Emola
                                     ...
                                     */
                                    //rollback nghiep vu cua ban hang
                                    logdetail.info("SaleTransToPackage: Error.Call.WS.Payment.Wallet" + saleTrans.getSaleTransId());
                                    this.getSession().clear();
                                    this.getSession().getTransaction().rollback();
                                    this.getSession().getTransaction().begin();
                                    //Update trang thai Sale Trans va Sale Trans Emola
                            ...
                            logdetail.info("SaleTransToPackage: Error. Can not update sale transaction! :" + saleTrans.getSaleTransId());
                            req.setAttribute(Constant.RETURN_MESSAGE, "Error. Can not Update sale  trans Emola transaction!");
                            this.getSession().clear();
                            this.getSession().getTransaction().rollback();
                            this.getSession().getTransaction().begin();
                            return pageForward;
                ...
                //tannh20180115 thuc hien nghiep vu  sale to pakege payment account transfer
...
//                }
                ...
                for (SaleTransDetailV2Bean bean : lstGoods) {
                    ...
                    //LinhNBV change saleTransId to saleTransOrderId
                    if (saleTransDetail == null || saleTransDetail.getSaleTransOrderId() == null) {
                        logdetail.info("SaleTransDetailOrderToRetail: SaleTransDetailOrderToRetail fail. saletransId" + saleTrans.getSaleTransId());
...
//                db.sendSms("258871233799", message.replace("XXX", staffCode).replace("YYY", orderCode), "86904");
            ...
            //ghi log
            logdetail.info("Exception  Method saveSaleTransToPackage :" + ex);
            this.getSession().clear();
            this.getSession().getTransaction().rollback();
...
//        getFileLocationToServer(fileDowd);
    ...
    private SaleTrans saveSaleTrans() throws Exception {
        ...
        saleTrans.setSaleTransDate(getSysdate());
        saleTrans.setSaleTransType(Constant.SALE_TRANS_TYPE_PACKAGE);
        saleTrans.setStatus(Constant.SALE_TRANS_STATUS_NOT_BILLED);
        saleTrans.setCheckStock("0");
    ...
    public String selectReasonSale() {
        ...
        try {
            if (this.saleTransForm.getReasonId() != null) {
                ...
                String reasonSaleForVIP = getReasonIdVipCustomer();
                ...
                //reset lai form vi mat hang co the co nhieu gia
                //ban cho khach hang VIP lay theo gia ban le
                //ban cho khach hang thuong lay gia ban theo goi
                saleTransForm = new SaleTransForm();
                saleTransForm.setSaleTransDate(getSysdate());
                saleTransForm.setReasonIdVipCustomer(getReasonIdVipCustomer());
                setTabSession(SESSION_LIST_GOODS, new ArrayList());
    ...
    private boolean checkExistedTransInMonthVIPCust(String msisdn) {
        ...
        try {
            ...
            q.setParameter(3, Constant.SALE_TRANS_TYPE_PACKAGE);
            List list = q.list();
    ...
    private boolean isSaleToVipCustomer() {
        if (saleTransForm != null) {
            String reasonIdVip = getReasonIdVipCustomer();
            if (reasonIdVip != null && !reasonIdVip.isEmpty()) {
                if (saleTransForm.getReasonId() != null && saleTransForm.getReasonId().equals(Long.valueOf(reasonIdVip))) {
                    return true;
                }
            }
        }
    ...
    private SaleTransVip saveSaleTransVip(SaleTrans saleTrans) throws Exception {
        ...
        saleTransVip.setSaleTransType(Constant.SALE_TRANS_TYPE_PACKAGE);
