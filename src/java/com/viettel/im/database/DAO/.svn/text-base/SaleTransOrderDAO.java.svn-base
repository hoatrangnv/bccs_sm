 ...
 */
public class SaleTransOrderDAO extends BaseDAOAction {

    private static final Log log = LogFactory.getLog(SaleTransOrderDAO.class);
    // property constants
    ...
    public static final String SALE_TRANS_ORDER_ID = "saleTransOrderId";
    public static final String SALE_TRANS_ID = "saleTransOrderId";
    ...
    public List<SaleTransBean> searchSaleTrans(Long shopId, String status) {
        ...
        if (!isSaleTransForSubShop()) {
            ...
            /**
             * Neu la lap hoa don ho dai ly thi khong tim kiem voi shopId =
             * shopId cua minh
             */
            sqlBuffer.append(" AND ");
    ...
    public SaleTransOrder findById(java.lang.Long id) {
        log.debug("getting SaleTrans instance with id: " + id);
        try {
            SaleTransOrder instance = (SaleTransOrder) getSession().get(
                    "com.viettel.im.database.BO.SaleTransOrder", id);
            return instance;
        } catch (RuntimeException re) {
            log.error("get failed", re);
            throw re;
    ...
    public List findByProperty(String propertyName, Object value) {
        ...
        try {
            String queryString = "from SaleTransOrder as model where model." + propertyName + "= ?";
            Query queryObject = getSession().createQuery(queryString);
    ...
    public List findAgentOrderByProperty(String propertyName, Object value) {
        log.debug("finding SaleTrans instance with property: " + propertyName + ", value: " + value);
        try {
            String queryString = "from AgentTransOrderHis as model where model." + propertyName + "= ?";
            Query queryObject = getSession().createQuery(queryString);
            queryObject.setParameter(0, value);
            return queryObject.list();
        } catch (RuntimeException re) {
    ...
    public SaleTransOrder findBySaleTransCode(Object saleTransCode) {
        List lst = findByProperty(SALE_TRANS_CODE, saleTransCode);
        if (lst.size() > 0) {
            return (SaleTransOrder) lst.get(0);
        }
        return null;
    }

    public SaleTransOrder findBySaleTransOrderId(Object saleTransOrderId) {
        List lst = findByProperty(SALE_TRANS_ORDER_ID, saleTransOrderId);
        if (lst.size() > 0) {
            return (SaleTransOrder) lst.get(0);
        }
        return null;
    ...
    public AgentTransOrderHis findAgentOrderBySaleTransOrderId(Object saleTransOrderId) {
        List lst = findAgentOrderByProperty(SALE_TRANS_ORDER_ID, saleTransOrderId);
        if (lst.size() > 0) {
            return (AgentTransOrderHis) lst.get(0);
        }
        return null;
    }

    public SaleTransOrder findBySaleTransId(Object saleTransId) {
        List lst = findByProperty(SALE_TRANS_ID, saleTransId);
        if (lst.size() > 0) {
            return (SaleTransOrder) lst.get(0);
        }
        return null;
    ...
    public SaleTransOrder findSaleTransOrderBySaleTransId(Object saleTransId) {
        List lst = findByProperty("saleTransId", saleTransId);
        if (lst.size() > 0) {
            return (SaleTransOrder) lst.get(0);
        }
        return null;
