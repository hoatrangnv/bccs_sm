<%-- 
    Document   : GetMoneyToAccountBalance
    Created on : Oct 23, 2009, 3:17:38 PM
    Author     : User
--%>
...
<s:token/>
    ...
    <tags:imPanel title="MSG.SIK.091">
        ...
        <table class="inputTbl8Col" style="width:100%">
            <tr>
                ...
                <%--<td class="label">Số tiền rút thêm (<font color="red">*</font>)</td>--%>
                <td class="label"><tags:label key="MSG.SIK.096" required="true"/></td>
                <td class="text">
    ...
    <tags:imPanel title="Payment note infomation">
        <table class="inputTbl6Col" style="width:100%">
            <tr>
                <%--<td class="label">Mã phiếu chi (<font color="red">*</font>)</td>--%>
                <td class="label"><tags:label key="MSG.SIK.097" required="true"/></td>
                <td class="text">
                ...
                <td class="text">
                    <tags:dateChooser property="AddMoneyToAccountBalanceForm.createDate"  id="createDate"  readOnly="true"  styleClass="txtInput"  insertFrame="false"/>
                </td>
            ...
            <tr>
                ...
                <td class="text">
                    <s:textfield name="AddMoneyToAccountBalanceForm.shopcode" id="shopcode" theme="simple" maxlength="1000" cssClass="txtInputFull"
                                 readonly="true"/>
                </td>
    ...
    <div align="center" style="padding-bottom:5px;">
        <s:if test="#session.print == 1">
            <%--<input type="button" onclick="getMoney();"  value="Rút tiền" disabled/>--%>
            <input type="button" onclick="getMoney();"  value="${fn:escapeXml(imDef:imGetText('MSG.SIK.089'))}" disabled/>
            <%--<input type="button" onclick="printReceipt();"  value="In phiếu chi" />--%>
            <input type="button" onclick="printReceipt();"  value="${fn:escapeXml(imDef:imGetText('MSG.SIK.101'))}" />
        </s:if>
        <s:else>
            <%--<input type="button" onclick="getMoney();"  value="Rút tiền"/>--%>
            <input type="button" onclick="getMoney();"  value="${fn:escapeXml(imDef:imGetText('MSG.SIK.089'))}"/>
            <%--<input type="button" onclick="printReceipt();"  value="In phiếu chi" disabled/>--%>
            <input type="button" onclick="printReceipt();"  value="${fn:escapeXml(imDef:imGetText('MSG.SIK.101'))}" disabled/>
        </s:else>
...
<script type="text/javascript" language="javascript">
    ...
    <%--$('messageShow').innerHTML="Bạn chưa nhập số tiền rút ra"--%>
//                $('messageShow').innerHTML='<s_:property value="getError('MSG.SIK.104')"/>';

                $('messageShow').innerHTML='<s:text name ="MSG.SIK.104"/>';

                moneyBalance.focus();
                return false;
            }
            if(trim(moneyBalance) == "0" ){
    <%--$('messageShow').innerHTML="Số tiền rút ra phải > 0"--%>
                $('messageShow').innerHTML='<s:property escapeJavaScript="true"  value="getError('MSG.SIK.105')"/>';
                moneyBalance.focus();
                return false;
            }
            if(!isInteger( trim(moneyBalance) ) ){
    <%--$('messageShow').innerHTML="Số tiền rút ra phải là số nguyên dương"--%>
                $('messageShow').innerHTML='<s:property escapeJavaScript="true"  value="getError('MSG.SIK.106')"/>';
                moneyBalance.focus();
                return false;
            }
            if(receiptCode.value==null || receiptCode.value ==""){
    <%--$('messageShow').innerHTML="Chưa nhập mã phiếu chi"--%>
                $('messageShow').innerHTML='<s:property escapeJavaScript="true"  value="getError('MSG.SIK.107')"/>';
                receiptCode.focus();
                return false;
            }
            if (isHtmlTagFormat(trim(receiptCode.value))){
    <%--$('messageShow').innerHTML="Mã phiếu chi không được nhập định dạng thẻ HTML"--%>
                $('messageShow').innerHTML='<s:property escapeJavaScript="true"  value="getError('MSG.SIK.108')"/>';
                receiptCode.focus();
                return false;
            }
            if(!isFormalCharacter(trim(receiptCode.value))){
    <%--$('messageShow').innerHTML="Mã phiếu chi không được nhập ký tự đặc biệt"--%>
                $('messageShow').innerHTML='<s:property escapeJavaScript="true"  value="getError('MSG.SIK.109')"/>';
                receiptCode.focus();
                return false;
            }
            if(reasonId.value==null || reasonId.value ==""){
    <%--$('messageShow').innerHTML="Chưa chọn lý do"--%>
                $('messageShow').innerHTML='<s:property escapeJavaScript="true"  value="getError('MSG.SIK.110')"/>';
                reasonId.focus();
                return false;
            }
    ...
    <%--if (!confirm("Bạn có chắc chắn muốn rút tiền không?")){--%>
            var strConfirm = getUnicodeMsg('<s:property escapeJavaScript="true"  value="getError('MSG.SIK.111')"/>');
            if (!confirm(strConfirm)){
                event.cancel = true;
            }
            document.getElementById("AddMoneyToAccountBalanceForm").action="CollAccountManagmentAction!getMoneyToAccountBalance.do?"+ token.getTokenParamString();
            document.getElementById("AddMoneyToAccountBalanceForm").submit();
        }
        resetForm = function(){
            document.getElementById("AddMoneyToAccountBalanceForm").action="CollAccountManagmentAction!resetgetAmount.do?"+ token.getTokenParamString();
            document.getElementById("AddMoneyToAccountBalanceForm").submit();
        }
        printReceipt = function(){
            document.getElementById("AddMoneyToAccountBalanceForm").action="CollAccountManagmentAction!printReceipt.do?printType=2"+"&"+ token.getTokenParamString();
            document.getElementById("AddMoneyToAccountBalanceForm").submit();
        }
