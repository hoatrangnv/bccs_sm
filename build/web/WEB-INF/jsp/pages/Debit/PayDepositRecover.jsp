...
<%-- 
    Document   : PayDepositRecover
    Created on : Sep 29, 2009, 10:29:19 AM
    Author     : Vunt
--%> 
...
<s:token/>
    ...
    <tags:imPanel title="MSG.search.import.note">
        <table class="inputTbl6Col">
            <tr>
                <td class="label"> <tags:label key="MSG.import.bill.code" /></td>
                <td class="text">
            ...
            <tr>
                ...
                <td class="text">
                    ...
                    <%--<s:select name="depositStatus" id="depositStatusId"
                              ...
                              list="#{'3':'Chưa lập phiếu','4':'Đã lập phiếu'}"
    ...
    <div id = "ReceiptBillsInfo" style="display:none">
        <sx:div id="listStockDetails">
            <s:if test="#request.listStockDetails != null && #request.listStockDetails.size() > 0">
                <jsp:include page="informationInOrderRecover.jsp" />
            </s:if>
            <br/>
        </sx:div>
        <div id = "ReceiptBillsInfoChild">
            <jsp:include page="receiptBillsInfoRecover.jsp" />
        </div>




        <!--jsp:include page="PayDepositReconverView.jsp" /-->
    ...
    <script type="text/javascript">
        ...
        <%--var _myWidget1 = dojo.widget.byId("autoFormId");
        ...
        dojo.html.hide("bttCancelInformation");--%>
            ...
            setDataTicket_old = function(a,id,stockTransId,depositStatus) {

                $( 'ReceiptBillsInfo' ).style.display ="block";

                var tr = a.parentNode.parentNode;
                var agentNameImport = tr.getElementsByTagName( 'td' )[4].getElementsByTagName( 'span' )[0].innerHTML;
                if(document.getElementById("agentNameImportId") != null){
                    document.getElementById("agentNameImportId").value= agentNameImport;
                    document.getElementById("agentId").value = document.getElementById("tempNameShopImportId_" + id).value;
                    document.getElementById("reasonName").value = document.getElementById("tempReasonNameId").value;
                    document.getElementById("stockTransIdHTML").value = stockTransId;
        ...
        event.cancel = true;--%>
                ...
                if(dateExported.domNode.childNodes[1].value.trim() != "" &&
                    ...
                    $('displayResultMsgClient').innerHTML= '<s:text name="ERR.DET.027"/>';
                    //$( 'displayResultMsgClient' ).innerHTML='Ngày trả từ không hợp lệ';
                ...
                if(dateExported1.domNode.childNodes[1].value.trim() != "" &&
                    ...
                    $('displayResultMsgClient').innerHTML= '<s:text name="ERR.DET.028"/>';
                    //$( 'displayResultMsgClient' ).innerHTML='Ngày trả đến không hợp lệ';
                ...
                if( !isCompareDate(dateExported.domNode.childNodes[1].value.trim(),dateExported1.domNode.childNodes[1].value.trim(),"VN")){
                    $('displayResultMsgClient').innerHTML= '<s:text name="ERR.DET.029"/>';
                    //$( 'displayResultMsgClient' ).innerHTML='Lập phiếu từ ngày phải nhỏ hơn đến ngày';
                    dateExported.domNode.childNodes[1].focus();
                    event.cancel = true;
                    return;
                }
                // TuTM1 04/03/2012 Fix ATTT CSRF
                widget.href = "PayDepositAction!searchCommandExportRecover.do" + "?" + token.getTokenParamString();
            });
            ...
            dojo.event.topic.subscribe("payDepositFormId/cancelInformationOrderAction", function(event, widget){
                widget.href = "saveInformationOrderAction!cancelInformationOrderActionRecover.do" + "?" + token.getTokenParamString();
            });

            dojo.event.topic.subscribe("payDepositFormId/saveInformationOrderAction", function(event, widget){
                ...
                widget.href = "saveInformationOrderAction!saveInformationOrderActionRecover.do" + "?" + token.getTokenParamString();
            });
