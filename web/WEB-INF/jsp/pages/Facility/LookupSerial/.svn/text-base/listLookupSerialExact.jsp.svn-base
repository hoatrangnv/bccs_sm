...
<%-- 
    Document   : listLookupSerialExact
    Created on : Sep 21, 2010, 9:17:00 AM
    Author     : Vunt
--%>
        ...
        <display:table name="listLookupSerialBean" id="tblListLookupSerialBean"
            ...
            <display:column title="${fn:escapeXml(imDef:imGetText('MSG.stock.type'))}" sortable="false" headerClass="tct">
                ...
                <s:elseif test="#attr.tblListLookupSerialBean.ownerType == 2">
                     <s:property escapeJavaScript="true"  value="getError('MSG.staff.stock')"/>
                </s:elseif>
            </display:column>
...
<!--                        -->
            ...
            <display:column title="${fn:escapeXml(imDef:imGetText('MSG.history.view'))}" sortable="false" headerClass="tct" style="width:80px; text-align:center; ">
                <s:a href="" onclick="aOnclick('%{#attr.tblListLookupSerialBean.serial}','%{#attr.tblListLookupSerialBean.stockModelCode}','%{#attr.tblListLookupSerialBean.status}')" cssClass="cursorHand">
                    <img src="${contextPath}/share/img/accept.png" title="<s:property escapeJavaScript="true"  value="getError('MSG.history.view')"/>" alt="<s:property escapeJavaScript="true"  value="getError('MSG.history.view')"/>"/>
                </s:a>
            </display:column>
        </display:table>
    </div>
