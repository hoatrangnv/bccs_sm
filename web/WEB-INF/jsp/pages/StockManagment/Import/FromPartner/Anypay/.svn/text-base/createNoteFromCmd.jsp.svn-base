                ...
                <table class="inputTbl3Col">
                    <tr>
                        ...
                        <s:if test="#request.isPrintMode == 'true'">
                            <input type="button" style="width: 100px;" value="${fn:escapeXml(imDef:imGetText('anypay.card.create'))}" disabled/>
                        </s:if>
                        <s:else>
                            <tags:submit id="imp"
                                         confirm="true"
                                         confirmText="MSG.confirm.create.card.anypay"
                                         formId="importStockForm"
                                         cssStyle="width: 100px;"
                                         showLoadingText="true"
                                         targets="divCreateImpNoteFromPartner"
                                         value="anypay.card.create"
                                         validateFunction="checkValidImportStockForm()"
                                         preAction="anypayPartnerAction!createNoteAnypayPartner.do"/>
                        </s:else>
                        </td>
...
<script>
    checkValidImportStockForm =function(){
        ...
        if(trim($('importStockForm.noteImpCode').value) == "") {
            ...
            $('importStockForm.noteImpCode').select();
            $('importStockForm.noteImpCode').focus();
            return false;
        }
