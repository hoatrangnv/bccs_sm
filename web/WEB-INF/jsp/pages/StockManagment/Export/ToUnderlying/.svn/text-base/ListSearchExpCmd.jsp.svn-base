...
<%
            ...
            request.getAttribute("lstSearchStockTrans");
%>
...
<s:if test="#request.lstSearchStockTrans != null && #request.lstSearchStockTrans.size()>0">
    ...
    <display:table id="trans" name="lstSearchStockTrans" class="dataTable" pagesize="15" targets="searchArea" requestURI="ExportStockUnderlyingAction!searchExpTrans.do" >
        ...
        <display:column title="${fn:escapeXml(imDef:imGetText('MSG.SIK.061'))}" style="width:100px" sortable="false" headerClass="tct">

            <s:if test="#attr.trans.hisAction.trim().length()  > 0 ">
                <s:property escapeJavaScript="true"  value="#attr.trans.hisAction.trim().substring(0,#attr.trans.hisAction.trim().length()-1)"/>
            </s:if>
            <s:else>
                ${fn:escapeXml(imDef:imGetText('MSG.GOD.190'))}                
            </s:else>
        </display:column>
        ...
        <display:column title="${fn:escapeXml(imDef:imGetText('MSG.GOD.207'))}" sortable="false" headerClass="tct">
            <div align="center">
                <s:if test="#attr.trans.stockTransStatus ==1"> <%-- chi nhung lenh chua lap phieu moi duoc lap phieu --%>
                    <s:url action="ExportStockUnderlyingAction!prepareCreateNoteFromCmd" id="URL" encode="true" escapeAmp="false">
