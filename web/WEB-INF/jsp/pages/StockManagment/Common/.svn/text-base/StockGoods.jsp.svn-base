...
<%
    ...
    if (request.getAttribute("isChoose") != null) {
        isChoose = Boolean.parseBoolean(request.getAttribute("isChoose").toString());
    }

    notEdit = !isEdit;
    String nonEdit = new Boolean(notEdit).toString();
    String edit = new Boolean(isEdit).toString();
    String choose = new Boolean(isChoose).toString();
    request.setAttribute("edit", edit);
    request.setAttribute("nonEdit", nonEdit);
    request.setAttribute("isChoose", choose);
...
<script>    
    ...
    <s:if test="#request.revokeColl =='true'">
            widget.href = "CommonStockAction!addGoods.do?revokeColl=true"+"&"+token.getTokenParamString();
    </s:if>
    <s:else>
            widget.href = "CommonStockAction!addGoods.do?"+token.getTokenParamString();
    </s:else>
        ...
        dojo.event.topic.subscribe("ExportStockForm/edit", function(event, widget){
            ...
            $('quantity').value=$('quantity').value.trim();
    <s:if test="#request.revokeColl =='true'">
            widget.href = "CommonStockAction!editGoods.do?revokeColl=true"+"&"+token.getTokenParamString();
    </s:if>
    <s:else>
        ...
        dojo.event.topic.subscribe("ExportStockForm/cacelEdit", function(event, widget){
            $('quantity').value ='';
    <s:if test="#request.revokeColl =='true'">
            widget.href = "CommonStockAction!cancelEditGoods.do?revokeColl=true";
    </s:if>
    <s:else>
        ...
        dojo.event.topic.subscribe("exportStockToCollaboratorAction/add", function(event, widget){
            ...
            $('quantity').value=$('quantity').value.trim();
            var shopImportedCode= document.getElementById("exportStockForm.shopImportedCode");
            widget.href = "CommonStockAction!addGoodsColl.do?shopImportedCode=" + shopImportedCode.value;
        });
        ...
        dojo.event.topic.subscribe("exportStockToCollaboratorAction/edit", function(event, widget){
            ...
            $('quantity').value=$('quantity').value.trim();
            var shopImportedCode= document.getElementById("exportStockForm.shopImportedCode");
            widget.href = "CommonStockAction!editGoodsColl.do?shopImportedCode=" + shopImportedCode.value;
        });
        ...
        <table class="inputTbl4Col">
            <tr>
                <s:if test="#attr.collaborator =='coll'">
                    ...
                    <td class="text" colspan="3">
                        <!--                        chi chon hang hoa trong kho nvql-->
                        ...
                        <tags:imSearch codeVariable="goodsForm.stockModelCode" nameVariable="goodsForm.stockModelName"
                                       ...
                                       postAction="selectStockModelColl()"
                                       otherParam="staff_export_type"
                                       readOnly="${fn:escapeXml(requestScope.edit)}"/>
                    </td>
                </s:if>
                <s:else>
                    <s:if test="#request.revokeColl =='true'">
                        <td class="label"><tags:label key="messages.stock.code"/></td>
                        <td class="text" colspan="3">
                            <tags:imSearch codeVariable="goodsForm.stockModelCode" nameVariable="goodsForm.stockModelName"
                                           codeLabel="MSG.GOD.007" nameLabel="MSG.GOD.008"
                                           searchClassName="com.viettel.im.database.DAO.CommonDAO"
                                           searchMethodName="getListStockModel"
                                           getNameMethod="getStockModelName"
                                           otherParam="staff_export_type;importStockForm.fromOwnerCode;importStockForm.fromOwnerType"
            ...
            <tr>
                <td colspan="4" align="center" class="text">
                    <s:if test="#attr.isChoose =='false'">
                        <tags:VTsx_submit parseContent="true" executeScripts="true"
                                          formId="goodsForm" loadingText="loading..." showLoadingText="true"
                                          targets="inputGoods"  value="MSG.GOD.019"  beforeNotifyTopics="exportStockToCollaboratorAction/add"
                        ...
                        <tags:VTsx_submit parseContent="true" executeScripts="true" formId="goodsForm"
                                          ...
                                          value="MSG.GOD.020"  beforeNotifyTopics="exportStockToCollaboratorAction/edit"
                                          errorNotifyTopics="errorAction" disabled="${fn:escapeXml(nonEdit)}"/>

                        <tags:VTsx_submit parseContent="true" executeScripts="true" formId="goodsForm"
                                          loadingText="loading..." showLoadingText="true" targets="inputGoods"
                                          value="MSG.GOD.018"  beforeNotifyTopics="exportStockToCollaboratorAction/cacelEdit"
                                          errorNotifyTopics="errorAction" disabled="${fn:escapeXml(nonEdit)}"/>
                    ...
                    <input type="button" onclick="viewStockDetail('<s:property escapeJavaScript="true"  value="#attr.URLViewStock"/>')" value='<s:text name="MSG.GOD.171"/>'/>
                    ...
                    <s:if test="#attr.collaborator =='coll'">
                        ...
                        <input type="button" onclick="viewAccountAgentDetail('<s:property escapeJavaScript="true"  value="#attr.URLViewAccountAgent"/>')" value='<s:text name="MSG.GOD.172"/>'/>
                    </s:if>
                    <s:else>
                        <s:if test="#request.revokeColl !='true'">
                            <s:url action="exportStockToCollaboratorAction!prepareViewAccountAgent" id="URLViewAccountShop" encode="true" escapeAmp="false">
                            </s:url>
                            <input type="button" onclick="viewAccountAgentShop('<s:property escapeJavaScript="true"  value="#attr.URLViewAccountShop"/>')" value='<s:text name="MSG.GOD.172"/>' />
                        </s:if>
                    </s:else>
                </td>
            </tr>
