...
<%-- 
    Document   : ListGoodsAgentDeposit
    Created on : Nov 25, 2010, 5:25:35 PM
    Author     : vunt
--%>
...
<sx:div id="localListGoods">
    <tags:imPanel title="MSG.detail.goods">
        <div style="height:210px; overflow:auto;">
            <tags:displayResult id="returnMsgGoodsClient" property="returnMsg" propertyValue="returnMsgValue"/>
            ...
            <s:set var="stockTypeId" value="#attr.good.stockTypeId" scope="request"/>
                            ...
                            <div align="center">
                                ...
                                <s:if test="#attr.good.lstSerial == null || #attr.good.lstSerial.size()==0">
                                    ...
                                    <sx:a targets="inputGoods" href="%{#URL2}" cssClass="cursorHand" executeScripts="true" parseContent="true">
                                        <img src="${contextPath}/share/img/edit_icon.jpg" title="<s:property escapeJavaScript="true"  value="getText(MSG.edit.goods)"/>" alt="<s:property escapeJavaScript="true"  value="getText(MSG.edit.goods)"/>"/>
                                    </sx:a>
                                </s:if>
                                <a onclick="deleteGoods('listGoods','<s:property escapeJavaScript="true"  value="#attr.URL3"/>')">
                                    <img src="${contextPath}/share/img/delete_icon.jpg" title="<s:property escapeJavaScript="true"  value="getText(MSG.delete.goods)"/>" alt="<s:property escapeJavaScript="true"  value="getText(MSG.delete.goods)"/>"/>
                                </a>
                        ...
                        <s:else>
                            <s:if test="#request.revokeColl =='true'">
                                <s:url action="CommonStockAction!prepareEditGoodsAgentDeposit" id="URL2">
                                    <s:param name="stockModelId" value="#attr.good.stockModelId"/>
                                    <s:param name="stockTypeId" value="#attr.good.stockTypeId"/>
                                    <s:param name="stateId" value="#attr.good.stateId"/>
                            ...
                            <%--  <s:if test="#request.revokeColl =='true'">
                              ...
                              <s:else>--%>
                            <s:url action="CommonStockAction!delGoodsAgentDeposit" id="URL3" escapeAmp="false">
                                <s:param name="stockModelId" value="#attr.good.stockModelId"/>
                                <s:param name="stockTypeId" value="#attr.good.stockTypeId"/>
                                <s:param name="stateId" value="#attr.good.stateId"/>
                            </s:url>
...
<script>
    ...
    deleteGoods=function(target, action){
        ...
        if(confirm(getUnicodeMsg('<s:property escapeJavaScript="true"  value="getText('MSG.STK.001')"/>'))){
    <s:if test="#request.revokeColl=='true'">
    ...
    --%>
        ...
        function checkAndOpenPopup(path, w, h){
            ...
            {
                alert(getUnicodeMsg( '<s:property escapeJavaScript="true"  value="getText('MSG.GOD.174')"/>'));
