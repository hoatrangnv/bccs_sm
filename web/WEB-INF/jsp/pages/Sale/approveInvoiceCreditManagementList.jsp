...
<div id="invoiceUsedList">
    <s:if test="#request.lstInvoice != null && #request.lstInvoice.size() != 0">
        <inventoryDisplay:table targets="invoiceUsedList" id="invoiceUsed"
            ...
            <display:column title="${imDef:imGetText('MSG.approveInvoice')}" sortable="false" headerClass="tct" style="text-align:center; width:50px;">
                <!--                  Duyet yeu cau-->
                <div id="a<s:property value="#attr.invoiceUsed.invoiceUsedId"/>">
                    <s:if test = "#attr.invoiceUsed.checkApproveInvoice*1 == 1">
                        <a href="#" onclick="approveInvoice('<s:property value="#attr.invoiceUsed.invoiceUsedId"/>')">
                            <img src="${contextPath}/share/img/accept.png" title="${imDef:imGetText('MSG.approveInvoice')}" alt="${imDef:imGetText('MSG.approveInvoice')}"/>
                        </a>
                    </s:if>
                    <s:elseif test = "#attr.invoiceUsed.checkApproveInvoice*1 == 2">
                        <s:property escapeJavaScript="true"  value ="getText('MSG.approved')"/>
                    </s:elseif>
                </div>

            </display:column>
            <display:column title="${imDef:imGetText('MSG.rejectInvoice')}" sortable="false" headerClass="tct" style="text-align:center; width:50px;">
                <!--                  Tu choi yeu cau-->
                <div id="b<s:property value="#attr.invoiceUsed.invoiceUsedId"/>">
                    <s:if test = "#attr.invoiceUsed.checkApproveInvoice*1 == 1">
                        <a href="#" onclick="rejectInvoice('<s:property value="#attr.invoiceUsed.invoiceUsedId"/>')">
                            <img src="${contextPath}/share/img/delete_icon.png" title="${imDef:imGetText('MSG.rejectInvoice')}" alt="${imDef:imGetText('MSG.rejectInvoice')}"/>
                        </a>
                    </s:if>
                    <%--<s:elseif test = "#attr.invoiceUsed.checkApproveInvoice*1 == 2">--%>
                    <%--<s:property escapeJavaScript="true"  value ="getText('MSG.approved')"/>--%>
...
//            $( 'returnMsgClient' ).innerHTML='Chưa chọn lý do huỷ HĐ!';
    ...
    aOnclick = function (invoiceUsedId) {
        openDialog('ApproveInvoiceCreditManagementAction!getInvoiceInfo.do?invoiceUsedId=' + invoiceUsedId, 1024, 768, false);
    };

    approveInvoice = function (invoiceUsedId) {
        var strConfirm = getUnicodeMsg('<s:property value="getText('MSG.approveInvoice')"/>');
        if (confirm(strConfirm)) {
            document.getElementById("a" + invoiceUsedId).style.display = 'none';
            document.getElementById("b" + invoiceUsedId).style.display = 'none';
            gotoAction('CreditInvoiceArea', "${contextPath}/ApproveInvoiceCreditManagementAction!approveCreditInvoice.do?invoiceUsedId=" + invoiceUsedId + "&" + token.getTokenParamString());
        }
    };
    rejectInvoice = function (invoiceUsedId) {
        var strConfirm = getUnicodeMsg('<s:property value="getText('MSG.rejectInvoice')"/>');
        if (confirm(strConfirm)) {
            document.getElementById("a" + invoiceUsedId).style.display = 'none';
            document.getElementById("b" + invoiceUsedId).style.display = 'none';
            gotoAction('CreditInvoiceArea', "${contextPath}/ApproveInvoiceCreditManagementAction!rejectCreditInvoice.do?invoiceUsedId=" + invoiceUsedId + "&" + token.getTokenParamString());
        }
    };
</script>