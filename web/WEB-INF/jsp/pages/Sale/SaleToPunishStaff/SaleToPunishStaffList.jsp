<%-- 
    Document   : SaleToPunishStaffList
    Created on : Dec 11, 2012, 6:07:15 PM
    Author     : trungdh3_s
--%>
    ...
    <div style="width:100%;overflow:auto; height:320px; vertical-align:top" >
        <s:if test="#request.lstGoods != null && #request.lstGoods.size() > 0">
            <display:table targets="goodList" id="goods" name="lstGoods" class="dataTable" cellpadding="1" cellspacing="1" requestURI="destroySaleInvoiceAction!pageNavigator.do" >
                ...
                <display:column title="${fn:escapeXml(imDef:imGetText('MSG.generates.goods.state'))}" sortable="false" style="text-align:left" headerClass="tct">
                    <s:if test="#attr.goods.state==1">
                        <s:text name = "MSG.GOD.new"/>
                    </s:if>
                    <s:if test="#attr.goods.state==2">
                        <s:text name = "MSG.GOD.old"/>
                    </s:if>
                    <s:if test="#attr.goods.state==3">
                        <s:text name = "MSG.GOD.error"/>
                    </s:if>
                </display:column>
                ...
                <%--display:column property="state" format="formatstate();" title="${fn:escapeXml(imDef:imGetText('MSG.SAE.038'))}" sortable="false" style="text-align:right" headerClass="tct"/>--%>
                        ...
                        <s:if test="#attr.goods.lstSerial != null && #attr.goods.lstSerial.size()>0">
                            <div align="center">
                                <s:url action="InputSerialAction!prepareInputSerial" id="URLView" encode="true" escapeAmp="false">
                                    ...
                                    <s:param name="stateId" value="#attr.goods.state"/>
                                    <s:param name="isView" value="true"/>
                                    <s:param name="purposeInputSerial" value="4"/>
...
<script type="text/javascript" language="javascript">
    ...
    prepareEditGoods = function(stockModelId){
        gotoAction("saleToPunishDetailArea",'${contextPath}/saleToPunishStaffAction!prepareEditGoods.do?stockModelId='+ stockModelId + "&" + token.getTokenParamString());
    }
    deleteGoods = function(stockModelId){
        gotoAction("saleToPunishDetailArea",'${contextPath}/saleToPunishStaffAction!deleteGoods.do?stockModelId='+ stockModelId + "&" + token.getTokenParamString());
    }

    updateSerial = function() {
        gotoAction("saleToPunishDetailArea",'${contextPath}/saleToPunishStaffAction!refreshGoodsList.do' + "?" + token.getTokenParamString());
    }
