    ...
    <fieldset class="imFieldset">
        ...
        <div style="height:350px; overflow:auto; ">
            <display:table name="invoiceList" id="invoice"
                ...
                <display:column property="blockNo" title="${fn:escapeXml(imDef:imGetText('MSG.bin.book.number'))}" sortable="false" headerClass="sortable" style="width:40px; text-align:right" escapeXml="true"/>
    ...
    <div class="divHasBorder" style="padding:3px; margin-top:5px; text-align:center;">
        <s:if test="form.ownerId != null && form.ownerId > 0 ">
            <tags:submit targets="bodyContent" formId="form"
                         ...
                         preAction="retrieveInvoiceListAction!retrieveBillComplete.do"
                         showLoadingText="true" validateFunction="validateRetrieve()"/>
        </s:if>
        <s:else>
            <input type="button" style="width: 80px; " disabled value="${fn:escapeXml(imDef:imGetText('MSG.recover'))}">
        </s:else>
    </div>
</sx:div >
